...
def get_job_details(job_id, mode):
    ...
    if search_results:
        ...
        jobs['background'] = 'url(' + search_results[
            'background_url'] + ')' if 'background_url' in search_results else None
    return {'jobs': jobs, 'tracks': track_results, 'search_results': search_results, 'success': True, 'mode': mode,
            'job id': job_id}
...
def process_logfile(logfile, job, job_results):
    ...
    if job.status == "ripping":
        ...
        job_results = process_makemkv_logfile(job, job_results, db)
