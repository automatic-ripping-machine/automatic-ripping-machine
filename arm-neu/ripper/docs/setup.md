...
This guide covers deploying ARM + UI + Transcoder on one or two machines using published Docker Hub images, and local development setup.
...
| `SKIP_TRANSCODE` | **`false` (default)**: ARM POSTs to `TRANSCODER_URL` and waits for the transcoder's callback. **`true`**: ARM finalizes output directly (moves raw files to `COMPLETED_PATH`) and marks the job SUCCESS with no transcoder handoff. |
